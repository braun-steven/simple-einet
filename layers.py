...
class AbstractLayer(nn.Module, ABC):
    ...
    @abstractmethod
    def sample(
        self, n: int = None, context: SamplingContext = None
...
class Sum(AbstractLayer):
    ...
    def __repr__(self):
        return (
            "Sum(in_channels={}, in_features={}, out_channels={}, dropout={}, out_shape={})".format(
...
class CrossProduct(AbstractLayer):
    ...
    def __init__(self, in_features: int, in_channels: int, num_repetitions: int = 1):
        ...
        self.out_shape = (
            f"(N, {self._out_features}, {self.in_channels ** 2}, {self.num_repetitions})"
        )
...
class EinsumLayer(AbstractLayer):
    ...
    def sample(
        ...
        # Check dimensions
        assert weights.shape == (N, D, IC2, IC2)
